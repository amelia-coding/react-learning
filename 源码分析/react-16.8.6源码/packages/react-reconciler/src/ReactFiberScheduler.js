...
function commitAllHostEffects() {
  while (nextEffect !== null) {
    ...
    // 判断不同的effectTag， 处理不同的side Effect
...
function flushPassiveEffects() {
  ...
  if (passiveEffectCallback !== null) {
    // 我们调用调度的回调而不是直接调用commitPassiveEffects来确保跟踪正常工作。
...
/**
 * commitBeforeMutationLifecycles 就是执行getSnapshotBeforeUpdate
 * commitAllHostEffects 执行DOM节点相关的操作
 * commitAllLifeCycles 执行组件实例相关操作
 * 把更改更新到页面上，执行相应的生命周期，此阶段不能打断，
 * 把finishdWork消费掉，通过判断Fiber的EffectTag 处理不同的更新 
 * @param {*} root 
 * @param {*} finishedWork 
 */
function commitRoot(root: FiberRoot, finishedWork: Fiber): void {
  ...
  markCommittedPriorityLevels(root, earliestRemainingTimeBeforeCommit); // 找到非Nowork的任务
  ...
  let firstEffect;
  // beginWork里，打上了不同的 effectTag
  if (finishedWork.effectTag > PerformedWork) {
    // A fiber's effect list consists only of its children, not itself. So if
    // the root has an effect, we need to add it to the end of the list. The
    // resulting list is the set that would belong to the root's parent, if
    // it had one; that is, all the effects in the tree including the root.
    // finishedWork如果有副作用就把finishedWork添加到链表的最后， 组合成一个最终要处理的链表
  ...
  // 真正的对 dom 进行操作
  //将所有副作用置于树中。 我们将分两次通过。第一遍执行所有主机插入，更新，删除和ref卸载。
  nextEffect = firstEffect;
  startCommitHostEffectsTimer();
  // 按链表执行完所有的effect
  while (nextEffect !== null) {
    ...
    if (__DEV__) {
      ...
      try {
        // commitAllHostEffects 中对不同 effectTag 进行不同操作， 操作真实的dom了
  ...
  // 在第二遍中，我们将执行所有生命周期和ref回调。
  // 生命周期作为单独的传递发生，因此整个树中的所有放置，更新和删除都已被调用。
  // 此传递还触发任何渲染器特定的初始效果。
  nextEffect = firstEffect;
  startCommitLifeCyclesTimer();
  while (nextEffect !== null) {
...
function performUnitOfWork(workInProgress: Fiber): Fiber | null {
  ...
  if (enableProfilerTimer) {
    ...
    // 执行beginWork进行节点操作，以及创建子节点
    // 如果子节点存在更新会一直执行下去，直到子节点不存在更新， 或遍历完所有子节点
    // 这个操作会做dom 对比， 最后打上标记
    // 子节点会返回成为next，继续遍历
    // 也就是说这个方法会把当前fiber下的所有子节点的副作用都求出来
  ...
  // 没有子节点了，任务完成, 在while中
   // 然后再循环performUnitOfWork，清空下一个fiber下所有的子节点任务
  if (next === null) {
...
function workLoop(isYieldy) {
  // 循环遍历求nextUnitOfWork，直到nextUnitOfWork为空 
  if (!isYieldy) {
    // Flush work without yielding
    // nextUnitOfWork 是 workInProgress tree
    // nextUnitOfWork  是一个fiber节点
    // performUnitOfWork  => 返回的是nextUnitOfWork节点的后续节点
    // 求出当前节点的所有后续节点
...
// 1. 作用是什么？
// 要把FiberRoot 下的child节点的更新求出来，finishWork 才能执行commit阶段
function renderRoot(root: FiberRoot, isYieldy: boolean): void {
  ...
  // 判断是否有新的更新进来，nextRenderExpirationTime，
  //  nextRoot发生变化 有新的更高优先级任务进来
  if (
  ...
  // 执行workLoop  循环
  // 在workLoop中，求出所有的副作用，得到finishWork
  do {
    try {
      workLoop(isYieldy);
    } catch (thrownValue) {
...
function computeExpirationForFiber(currentTime: ExpirationTime, fiber: Fiber) {
  ...
  if ((fiber.mode & ConcurrentMode) === NoContext) {
    ...
    // 在 renderRoot 阶段，优先级设置为当前正在工作的fiber的到期时间
...
(fiber: Fiber, expirationTime: ExpirationTime) {
  ...
  if (
    !isWorking &&
    // nextRenderExpirationTime 在初始的时候是noWork， 被设置后不再是noWork， 当前有任务
    nextRenderExpirationTime !== NoWork &&
    // expirationTime > nextRenderExpirationTime  新来的任务优先级高
    expirationTime > nextRenderExpirationTime
  ) {
...
// 判断上一个任务能否被打断，不能被打断，就return
// 能被打断，就cancle掉上一个任务，重新建一个新的任务，进行异步调度
function scheduleCallbackWithExpirationTime(
  root: FiberRoot,
  expirationTime: ExpirationTime,
) {
  // 判断是否还有正在跑的任务，为true则说明上一个fiber任务还没有执行完，还在执行
  if (callbackExpirationTime !== NoWork) {
    // A callback is already scheduled. Check its expiration time (timeout).
    if (expirationTime < callbackExpirationTime) {
      // Existing callback has sufficient timeout. Exit.
      // 现有的callBack还有时间，继续执行，所以return
      return;
    } else {
      // 优先级更高的任务来了
      // 清空之前的callbackID
      if (callbackID !== null) {
        ...
        // new one.
        // 现有的回调没有足够的时间了。 取消现在的任务并安排一个新的任务。
...
// requestWork干活的地方，执行requestWork的地方有很多， 
// 通过这几个参数的设置情况来决定是立即执行还是等待
...
// 1. requestWork 分为同步任务和异步任务，进入performWork
// 2. 找到最高优先级的任务
// 3. 存在任务 performWorkOnRoot
// 4. performWorkOnRoot里判断finishedWork是否存在
// 5. finishedWork不存在，走workLoop  通过beginWork循环求出子节点的update
// 6. 得到finishedWork， 执行commit更新到页面
function performWork(minExpirationTime: ExpirationTime, isYieldy: boolean) {
  ...
  // nextFlushedRoot = highestPriorityRoot;
  // nextFlushedExpirationTime = highestPriorityWork;
  // 找出优先级最高的root，同时存在多个FiberRoot，这个是存在的
  // nextFlushedRoot 就是我们找到的最高优先级的任务
  findHighestPriorityRoot();
  // 是否能打断
  // performSyncWork  同步任务  不能打断
  // performAsyncWork 异步任务  可以打断
  if (isYieldy) {
    // 重新计算时间
    recomputeCurrentRendererTime();
    currentSchedulerTime = currentRendererTime;
    ...
    // 存在任务 & 任务未完成 & 任务超时  => 执行performWorkOnRoot
    // didYield表示已经被叫停，任务完成。
    // currentRendererTime > nextFlushedExpirationTime  时间还够
    while (
...
function performWorkOnRoot(
  ...
  if (!isYieldy) {
    ...
    // update队列 => updateQuene 
    let finishedWork = root.finishedWork;
    if (finishedWork !== null) {
      // This root is already complete. We can commit it.
      // 存在任务了
      // finishedWork 存在，进行commit了,就是把副作用更新到页面
...
function completeRoot(
  ...
  // commit阶段不能被打断，commit阶段的优先级设置为最高等级
...
// the reconciler. 
// setState => requestWork => isBatchingUpdates=true 导致直接return

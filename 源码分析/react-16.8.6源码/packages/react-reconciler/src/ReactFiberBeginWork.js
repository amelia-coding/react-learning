...
export function reconcileChildren(
  ...
  if (current === null) {
    ...
    // 实例化成一个Fiber对象，最终赋值给workInProgress.child
    // 走创建的逻辑，不用通过复杂的fiber 树的对比的逻辑
    ...
    // let's throw it out.
    // 实例化成一个Fiber对象，最终赋值给 workInProgress.child
...
function updateClassComponent(
  ...
  if (instance === null) {
    ...
    // 构造实例  new Component()  constructor
    constructClassInstance(
      workInProgress,
      Component,
      nextProps,
    ...
    // callComponentWillReceiveProps 会调  UNSAFE_componentWillReceiveProps
    // componentWillMount
    // checkShouldComponentUpdate,如果有更新的话，调用 UNSAFE_componentWillUpdate
  ...
  // 会执行render 函数了, 返回的是下一个fiber实例给nextUnitOfWork，继续循环，直到全部
...
function mountLazyComponent(
  ...
  // 默认status是-1，执行传入的方法得到thenable方法，
  // 根据thenable的结果更新组件的状态，并且把resolve的组件进行返回
  let Component = readLazyComponentType(elementType);
  // Store the unwrapped component in the type.
  // 他把lazy组件的Fiber对象的tag和type都改变了，
  // 下次执行到这个Fiber的更新，则会直接更新返回的type
...
function mountIndeterminateComponent(
  ...
  if (
    ...
    // ClassComponent不存在hookd， 所以reset
    resetHooks();
    ...
    // 执行getDerivedStateFromProps 这个生命周期
...
function beginWork(
  ...
  // workInProgress tree 上一个fiber更新了之后留下的一个镜像。
  // workInProgress & Fiber  => 对比操作性能会更好
  const updateExpirationTime = workInProgress.expirationTime;
  // 判断节点是否存在, 存在则说明已经渲染过了，
  // 这个过程就是对比current 和 workInProgress，求出diff
  if (current !== null) {
  ...
  // 就根据不同的节点类型进行更新
  // 真正在这个下面的逻辑里面打上标签EffectTag, update => updateQuene => finishWork
  switch (workInProgress.tag) {
